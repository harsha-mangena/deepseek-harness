...
Surface events (`system/message`, `developer/message`, `user/message`, `assistant/message`, `tool/result`) require `surfaceOp` in both typed events and append input. A replacement uses exactly `{ op: 'replace', startSeq, endSeq }`, with inclusive `SessionSeq` endpoints in current surface order. An Assistant message embeds its exact compact provider stream and forbids `sourceEventSeqs`. Known log-only events forbid both metadata fields and never produce a message. A log-only append may instead pass an `InformationalEventIntent` (`{ ignorable: true }`), which stamps `ignorable: true` on the persisted event envelope so builds that do not recognize the event type skip it instead of refusing the log; the marker is unavailable for surface events, which are model-visible and never skippable.
